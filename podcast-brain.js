...
const nav=[['home','⌂','Today'],['prepare','✎','Prepare'],['record','●','Studio'],['wrap','✓','Wrap'],['vault','◇','Vault']];
...
function setView(id){
  if(activeSession()&&!['record','wrap'].includes(id)){toast('Active recording session is locked');id='record'}
...
function buildNav(){
  ...
  const mobileSideLaunch=el('mobileSideLaunch');
  if(mobileSideLaunch)mobileSideLaunch.onclick=()=>{
    setView('record');
    enterMobileRecordingMode();
  };
}
...
function markerIcon(type){
  return {'Funny':'😂','Highlight':'❤️','Cut':'✂️','Sensitive':'⚠️','Future Episode':'💡','Hotline Callback':'📞','Running Joke':'🔁','Edit Note':'📝','Best Moment':'⭐'}[type]||'•';
}
function producerStrip(){
  if(!state.mustMentions.length)return '<span class="muted">No must-mentions added.</span>';
  return state.mustMentions.map((item,index)=>`<button class="producer-chip ${item.done?'done':''}" data-producer="${index}">${item.done?'✓':'○'} ${esc(item.text)}</button>`).join('');
}
function heatmap(){
  const types=['Funny','Highlight','Best Moment','Cut','Sensitive'];
  const counts=types.map(type=>state.markers.filter(marker=>marker.type===type).length);
  const max=Math.max(1,...counts);
  return types.map((type,index)=>`<div class="heat-row"><label>${esc(type)}</label><div class="heat-track"><div class="heat-fill" style="width:${Math.round((counts[index]/max)*100)}%"></div></div><b>${counts[index]}</b></div>`).join('');
}


function remoteMarkerButtons(){
  const order=[
    ['😂','Funny','Funny'],
    ...
    ['⭐','Best Moment','Best'],
    ['✂️','Cut','Cut'],
    ['⚠️','Sensitive','Sensitive'],
    ['📝','Edit Note','Note'],
    ['🔁','Running Joke','Running Joke'],
    ['📞','Hotline Callback','Hotline'],
    ['💡','Future Episode','Future']
  ];
  return order.map(([icon,type,label])=>`<button class="remote-marker" data-marker="${type}" aria-label="${label} marker"><div>${icon}</div><span>${label}</span></button>`).join('');
}
function record(){
...
function timeline(){
  return state.markers.length?state.markers.slice().reverse().map((marker,index)=>{
    const actualIndex=state.markers.length-1-index;
    return `<div class="timeline-polished"><div class="timeline-polished-icon">${markerIcon(marker.type)}</div><div class="timeline-polished-copy"><b>${esc(marker.type)}</b><small>${esc(marker.segment||'')}</small>${marker.note?`<span>${esc(marker.note)}</span>`:'<small>No note added</small>'}<button data-note-marker="${actualIndex}">${marker.note?'Edit note':'Add note'}</button></div><div><div class="timeline-polished-time">${fmt(marker.time)}</div><button class="btn" data-del-marker="${actualIndex}" style="margin-top:7px;padding:7px 9px">×</button></div></div>`;
...
function bindRecord(){
  ...
  const remoteUndo=el('remoteUndo');if(remoteUndo)remoteUndo.onclick=()=>{undoLastMarker();hideMarkerTray()};
  ...
  document.querySelectorAll('[data-producer]').forEach(button=>button.onclick=()=>{
    const item=state.mustMentions[Number(button.dataset.producer)];
    item.done=!item.done;
    save();
    record();
  });
...
function finishRecording(){const modal=document.createElement('div');modal.className='modal';modal.innerHTML=`<div class="modal-panel"><div class="section-head"><div><div class="eyebrow">Finish session review</div><h2>Before you stop recording…</h2></div></div>${finishReviewHtml()}<div class="button-row" style="margin-top:18px"><button class="btn" id="returnRecording">Return to Recording</button><button class="btn danger" id="finishAnyway">Finish Anyway</button></div></div>`;document.body.appendChild(modal);modal.querySelector('#returnRecording').onclick=()=>modal.remove();modal.querySelector('#finishAnyway').onclick=()=>{modal.remove();endSessionAndWrap()}}
function syncTimer(){
...
function buildEditingNotes(){
  ...
  lines.push(
    ...
    'WRAP NOTES',
    '----------',
    `Episode rating: ${state.wrap.rating||''}`,
